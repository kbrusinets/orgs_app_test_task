...
class Organization(Base):
    ...
    categories: Mapped[Set["Category"]] = relationship(
        secondary="organization_category", back_populates="organizations"
    )
...
class Address(Base):
    ...
    coordinates: Mapped[str] = mapped_column(
        Geography(geometry_type="POINT", srid=4326), unique=True
    )
...
class Category(Base):
    ...
    organizations: Mapped[Set["Organization"]] = relationship(
        secondary="organization_category", back_populates="categories"
    )
