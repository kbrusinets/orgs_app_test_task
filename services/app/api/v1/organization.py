...
@router.get(
    "/coords", summary="Get organizations by coords", response_model=List[OrgFull]
)
...
@router.get(
    "/id", summary="Get organization by id", response_model=Union[OrgFull, None]
)
async def get_by_id(
    user: Annotated[User, Depends(check_api_key)],
    backend: Annotated[Backend, Depends(get_backend)],
...
async def get_by_categories(
    ...
    return await backend.org_module.get_by_categories(
...
@router.get(
    "/radius", summary="Get organizations inside radius", response_model=List[OrgFull]
)
async def get_by_radius(
    ...
    return await backend.org_module.get_by_radius(
...
async def get_by_area(
    ...
    return await backend.org_module.get_by_area(
        lon=lon, lat=lat, height=height, width=width
    )
